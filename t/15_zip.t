...
ok( length(zipb64($s,$d)) / length(zipb64($s)) < 0.8,            '<0.8' );
my $f;
...
#--gzip, gunzip
$s=join'',map random([qw/hip hop and you do not everever stop/]), 1..2000;
my($gzl,$l)=map length, gzip($s), $s;
my $p=100*$gzl/$l;
ok($p < 25, sprintf'gziped length (%d) less than 1/4 (%d%%) of length (%d)',$gzl,$p,$l);
ok($s eq gunzip(gzip($s)),'gunzip(gzip())');
ok($s eq unzipbin(gunzip(gzip(zipbin($s)))),'unzipbin(gunzip(gzip(zipbin())))');
